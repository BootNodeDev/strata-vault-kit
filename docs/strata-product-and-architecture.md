...
| Governance keys        | Malicious upgrade                              | Timelock on every code change and on the delay itself, with a minimum that is never shorter than the notice, so an exit can complete inside the window. A pause running alongside the delay does close that exit for its duration; the timelock clock does not stop for it. Pausing is the guardian's and unpausing is governance's, so waiting out a delay behind a pause takes two authorities acting together, not one                                   |
